...
describe("Spectral-6 zone partition", () => {
  ...
  it("labels compass relative to origin", () => {
    expect(compassLabel({ longitude: -77.04, latitude: -11.94 }, { longitude: -77.045, latitude: -11.945 })).toBe(
      "NE",
    );
    expect(compassLabel({ longitude: -77.045, latitude: -11.945 }, { longitude: -77.045, latitude: -11.945 })).toBe(
      "centro",
    );
  });
...
describe("GetParcelSpectralZones", () => {
  ...
  it("returns live CDSE zone means when Statistical API succeeds per cell", async () => {
    ...
    const fetchFn = vi.fn().mockImplementation(async (url: string) => {
      ...
      if (String(url).includes("/statistics")) {
        statsCalls += 1;
        // Vary NIR slightly so tiers are not identical.
