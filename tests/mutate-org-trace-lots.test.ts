...
describe("Trace-2: mutate org trace lots", () => {
  ...
  it("creates coffee lot linked to org parcel", async () => {
    const registry = new OfflineTraceLotRegistry();
    const useCase = new CreateOrgTraceLot(registry, new SyntheticParcelRegistry());
    const result = await useCase.execute({
      authority: entitled,
  ...
  it("appends event with actorId and sets exported status", async () => {
    const registry = new OfflineTraceLotRegistry();
  ...
  it("hides not_found for foreign lotId", async () => {
    ...
    // Same org fixture lot should succeed
    expect(result.ok).toBe(true);
