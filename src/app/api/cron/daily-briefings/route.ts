...
import { runDailyBriefingDelivery } from "@/infrastructure/container";
...
async function handleCron(request: Request) {
  ...
  try {
    const result = await runDailyBriefingDelivery.execute();
    return NextResponse.json({ status: "OK", data: result });
  } catch (error) {
    console.error("daily briefing delivery cron failed", error);
    return NextResponse.json(
      {
        status: "CRON_FAILED",
